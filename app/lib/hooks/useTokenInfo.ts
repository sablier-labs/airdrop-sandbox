...
import { erc20Abi } from "@/app/lib/contracts/abi";
...
export function useTokenInfo(tokenAddress?: Address): UseTokenInfoReturn {
  ...
  const configQuery = useQuery({
    queryFn: async () => {
      ...
      return {
        ...
        totalDistribution: config.distribution.totalAmount,
  ...
  const contractAddress = tokenAddress || configQuery.data?.tokenAddress;
  ...
  const { data, error, isLoading, isSuccess, refetch } = useReadContracts({
    ...
    query: {
      enabled: !!contractAddress && !!configQuery.data,
      staleTime: 5 * 60 * 1000, // 5 minutes
    },
  });
  ...
  const tokenInfo = useMemo((): TokenInfo | undefined => {
    if (!data || !contractAddress || !configQuery.data) return undefined;
    ...
    // Get campaign metadata from config
    const config = configQuery.data;
    const totalDistribution = BigInt(config.totalDistribution);
    const averageAmount =
      config.totalRecipients > 0 ? totalDistribution / BigInt(config.totalRecipients) : 0n;
  ...
  }, [data, contractAddress, configQuery.data]);
  ...
  const enhancedError = useMemo(() => {
    ...
    return null;
  }, [configQuery.error, contractAddress, chainId, error, data]);
  ...
  return {
    ...
    isSuccess: isSuccess && !!tokenInfo && !configQuery.error,
    parseAmount,
    refetch: () => {
      configQuery.refetch();
      refetch();
